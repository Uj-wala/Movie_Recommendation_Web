...
def _ensure_unique_name(
    ...
    q = db.query(Collection).filter(Collection.user_id == user_id, Collection.name == name)
...
def create_collection(db: Session, user_id: str, data: CollectionCreate) -> Collection:
    ...
    collection = Collection(user_id=user_id, name=name, description=data.description)
    db.add(collection)
    db.commit()
...
def add_movie(
    ...
    movie = CollectionMovie(collection_id=collection.id, **data.model_dump())
    db.add(movie)
    db.commit()
    db.refresh(movie)
    return movie
