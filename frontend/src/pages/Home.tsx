  ...
  return (
    <div>
          ...
          <>
            ...
            <div className="absolute bottom-0 left-0 max-w-xl p-6 sm:p-10">
              ...
              <div className="mt-5 flex gap-3">
                <Link
                  to={`/movie/${hero.slug}`}
                  className="inline-flex items-center gap-2 rounded-lg gradient-primary px-5 py-2.5 font-semibold text-white"
                >
                  <Play size={18} className="fill-white" /> Watch Trailer
                </Link>
                <Link
                  to={`/movie/${hero.slug}`}
                  className="inline-flex items-center gap-2 rounded-lg border border-white/25 px-5 py-2.5 font-semibold hover:bg-white/5"
                >
      ...
      {Object.entries(rows).map(([title, movies]) => (
        <MovieRow key={title} title={title} movies={movies} loading={loading} />
      ))}
    </div>
  );
}
